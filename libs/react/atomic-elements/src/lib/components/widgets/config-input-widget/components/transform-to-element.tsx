...
import { ListItem, StatisticalDataType, Transformation, Variant } from '@kleeen/types';
  ...
  const getAutocompleteInput = (): JSX.Element => {
    ...
    return (
      <KsAutocomplete
        ...
        options={autoCompleteValues?.data || []}
        renderOption={(option) => (
          <TextFormatter format={format} formatType={formatType} transformation={Transformation.SelfSingle}>
            {option.displayValue}
          </TextFormatter>
        )}
        getOptionLabel={({ displayValue }) => (displayValue ? String(displayValue) : '')}
