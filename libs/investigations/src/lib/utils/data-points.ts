import { DataPointWithFormattedValue, Transformation } from '@kleeen/types';
...
export function getContextDataPoints({
  ...
  return dataPoints.filter((dataPoint) => {
    return (
      ...
      isSingleCardinalityTransformation(dataPoint.attribute?.aggregation as Transformation)
    );
  });
}
