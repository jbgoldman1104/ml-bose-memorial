...
export class CognitoAuthenticationHandler extends AuthenticationHandler {
  ...
  async signIn(options: SignInOptions): Promise<IUser> {
    ...
    if (!user) {
      throw new Error('The user credentials are invalid');
    }
    if (user.challengeName === AuthChallengeName.NEW_PASSWORD_REQUIRED) {
      throw {
        name: AuthException.NewPasswordRequiredException,
        message: 'You have to define a new password to continue',
      };
    }
