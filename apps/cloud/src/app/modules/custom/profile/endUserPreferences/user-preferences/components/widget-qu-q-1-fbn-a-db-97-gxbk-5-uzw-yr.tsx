...
function ThemeInputWidgetBase({ translate, ...widget }: CustomWidgetProps & TranslateProps) {
  ...
  const selectedTheme = find<Option>(propEq('id', theme.kit), options) || defaultTheme;
  ...
  function onSave() {
    const foundLanguageOption = find<Option>(propEq('value', localThemeRef.current), options) || defaultTheme;
    return {
